...
HFSSegment::HFSSegment(CStr modelPath)
{
	constructEngine();
...
#pragma omp parallel for
	for (int n = 0; n < trainNum; n++) {
		...
		getSegmentationI(seg, lab3u, mag1u, idx_mats[n], num_css[n], 0.35f, 0);
...
void HFSSegment::runDataSet(float c, int min_size)
{
	...
	for (int i = 0; i < testNum; i++) {
		...
		getSegmentationI(tmp, lab3u, mag1u, idx_mat, num_css[i], 0.05f, 80);
		getSegmentationII(segs[i], img3u, lab3u, mag1u, tmp, num_css[i], c, min_size);// 0.1f, 120);
		//Mat hed = imread(_bsds.WkDir + format("HED/test_nms_result/%d.png", atoi(_S(_bsds.testSet[i]))));
		//getSegmentationIII(segs[i], hed, segs0[i], num_css[i], 65.0f);
	}
	sdkStopTimer(&my_timer);
...
int HFSSegment::processImage(Mat &seg, Mat &img3u, float c, int min_size)
{
	...
	getSegmentationI(tmp, lab3u, mag1u, idx_mat, num_css, 0.05f, 80);
